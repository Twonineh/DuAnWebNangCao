...
.nav-item a:hover {
  ...
  box-shadow: 0px 0px 10px 2px #ffd700; /* Hiệu ứng ánh sáng bao quanh */
}
</style>
  <div class="container">
    <div class="row display-flex" style="background-color: white;">
      <div class="col-md-2 margin-auto trigger-menu">
        ...
        <div class="logo">
          <h2 style=" font-size: 35px; padding-bottom: 10px" class="logo-wrapper" href="{{ route('home_page') }}" title="{{ config('app.name') }}"><strong>Store</strong></h2>
        </div>
      </div>
      ...
      <div class="col-md-7 hd-bg-white main-menu-responsive" style="padding-top: 10px;">
        <div class="main-menu">
          <div class="nav">
            <ul style="display: flex; justify-content: space-around; padding: 10px; border-radius: 15px; background-color: #f8f9fa;">
              <li class="nav-item {{ Helper::check_active(['home_page']) }}">
                <a href="{{ route('home_page') }}" title="{{ __('header.Home') }}" style="padding: 10px 15px; border-radius: 10px; color:black">
                  <span class="fas fa-home"></span>
                  {{ __('header.Home') }}
                </a>
              </li>
              <li class="nav-item {{ Helper::check_active(['about_page']) }}">
                <a href="{{ route('about_page') }}" title="{{ __('header.About') }}" style="padding: 10px 15px; border-radius: 10px;">
                  <span class="fas fa-info"></span>
                  {{ __('header.About') }}
                </a>
              </li>
              <li class="nav-item dropdown {{ Helper::check_active(['products_page', 'producer_page', 'product_page']) }}">
                <a href="{{ route('products_page') }}" title="{{ __('header.Products') }}" style="padding: 10px 15px; border-radius: 10px;">
                  <span class="fas fa-mobile-alt"></span>
                  {{ __('header.Products') }}
                </a>
              </li>
              <li class="nav-item {{ Helper::check_active(['posts_page', 'post_page']) }}">
                <a href="{{ route('posts_page') }}" title="{{ __('header.News') }}" style="padding: 10px 15px; border-radius: 10px;">
                  <span class="far fa-newspaper"></span>
                  {{ __('header.News') }}
                </a>
              </li>
              <li class="nav-item {{ Helper::check_active(['contact_page']) }}">
                <a href="{{ route('contact_page') }}" title="{{ __('header.Contact') }}" style="padding: 10px 15px; border-radius: 10px;">
          ...
          <div class="accout-menu">
            @if(Auth::guest())
              <div class="not-logged-menu">
                <ul>
                  <li class="menu-item {{ Helper::check_active(['login']) }}">
                    <a href="{{ route('login') }}" title="{{ __('header.Login') }}">
                      <span class="fas fa-user"></span>
                      {{ __('header.Login') }}
                    </a>
                  </li>
                  <li class="menu-item {{ Helper::check_active(['register']) }}">
                    <a href="{{ route('register') }}" title="{{ __('header.Register') }}">
                ...
                <ul>
                  <li class="menu-item dropdown {{ Helper::check_active(['orders_page', 'order_page', 'show_user', 'edit_user']) }}">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      @endif
                      <li>
                        <a id="logout" action="#"><i class="fas fa-power-off"></i> {{ __('header.Logout') }}</a>
                      </li>
                    </ul>
...
<div class="mobile-main-menu">
  ...
  <div class="mobile-main-menu-middle">
    ...
    <div class="mb-menu-middle-body">
      <ul>
        <li class="mb-nav-item {{ Helper::check_active(['home_page']) }}">
          <a href="{{ route('home_page') }}" title="{{ __('header.Home') }}">
            <span class="fas fa-home"></span>
            {{ __('header.Home') }}
          </a>
        </li>
        <li class="mb-nav-item {{ Helper::check_active(['about_page']) }}">
          <a href="{{ route('about_page') }}" title="{{ __('header.About') }}">
            <span class="fas fa-info"></span>
            {{ __('header.About') }}
          </a>
        </li>
            ...
            <ul>
              @foreach($producers as $producer)
                <li class="{{ Helper::check_param_active('producer_page', $producer->id) }}">
                  <a href="{{ route('producer_page', ['id' => $producer->id]) }}" title="{{ $producer->name }}">
                    {{ $producer->name }}
                  </a>
                </li>
              @endforeach
        ...
        <li class="mb-nav-item {{ Helper::check_active(['posts_page', 'post_page']) }}">
          <a href="{{ route('posts_page') }}" title="{{ __('header.News') }}">
            <span class="far fa-newspaper"></span>
            {{ __('header.News') }}
          </a>
        </li>
        <li class="mb-nav-item {{ Helper::check_active(['contact_page']) }}">
          <a href="{{ route('contact_page') }}" title="{{ __('header.Contact') }}">
            <span class="fas fa-id-card"></span>
            {{ __('header.Contact') }}
          </a>
        </li>
        @if(Auth::check() && !Auth::user()->admin)
        <li class="mb-nav-item {{ Helper::check_active(['orders_page', 'order_page']) }}">
          <a href="{{ route('orders_page') }}">
            <span class="fas fa-clipboard-list"></span> Đơn Hàng
          </a>
        </li>
        @endif
      </ul>
